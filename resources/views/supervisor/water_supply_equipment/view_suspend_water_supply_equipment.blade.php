...
<div class="min-height-200px">
    <div class="mt-1 mb-5">
        <div class="mb-5">
            <h4>View All Water Supply Equipment (Suspend)</h4>
        </div>
    </div>
    <div class="card-box mb-30" style="overflow-y: auto;">
        <div class="col-sm-12">
            <table class="data-table table stripe hover nowrap dataTable no-footer dtr-inline" id="table">
                ...
                <tbody>
                @if (is_string($viewSuspendWaterSupplyEquipment))
                    <tr>
                        <td colspan="5" style="text-align: center; font-size: 1.5rem;">{{ $viewSuspendWaterSupplyEquipment }}</td>
                    </tr>
                @else
                @php
                    $counter = 0;
                @endphp
                @foreach ($viewSuspendWaterSupplyEquipment as $value)
                    <tr>
                        ...
                        <td>
                            <a href="{{ url('supervisor/water_supply_equipment/activate_water_supply_equipment/' . $value->water_supply_equipment_id) }}" class="btn btn-sm btn-primary">Activate</a>
                        </td>
                    </tr>
                @endforeach
            ...
            <div style="padding: 10px; float: right;">
                {!! $viewSuspendWaterSupplyEquipment->appends(Illuminate\Support\Facades\Request::except('page'))->links() !!}
            </div>
            @endif
        </div>
    </div>
