            ...
            <table class="data-table table stripe hover nowrap dataTable no-footer dtr-inline" id="table">
                ...
                @foreach ($viewCustomerMeterReading as $value)
                    <tr>
                        ...
                        <td>
                            <a href="{{ url('supervisor/report_analysis/view_individual_customer_meter_reading/'. $value->customer_id) }}" class="btn btn-sm btn-primary" style="font-size: 1.5rem;">View More</a>
                        </td>
                    </tr>
                @endforeach
                @endif
...
<script>
    $(document).ready(function() {
        $(document).on('keyup', '.search-input', function() {
            ...
            $.ajax({
                type: 'GET',
                url: '{!! URL::to('autoSearchCustomerMeterSupervisor') !!}',
                ...
                success: function(data) {
                    if (data && data.length > 0)  {
                        ...
                        $.each(data, function(index, customer) {
                            ...
                            html += '<td>';
                            html += '<a href="{{ url("supervisor/report_analysis/view_individual_customer_meter_reading") }}/' + customer.customer_id + '" class="btn btn-sm btn-primary" style="font-size: 1.5rem;">View More</a>';
                            html += '</td>';
                            html += '</tr>';
                        });
                        $('.fill_data').html(html);
