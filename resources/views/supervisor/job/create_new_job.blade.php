    ...
    <div class="pd-20 card-box mb-30" style="overflow-y: auto;">
        <form method="post" action="">
            ...
            <div class="form-group" data-select2-id="1">
                <label style="font-size: 1.5rem;">Employee Name <span style="color:red;">*</span></label>
                <select class="form-control custom-select2 select2-hidden-accessible" name="employee_id" required style="width: 100%" {{--  multiple="" --}} data-select2-id="1" tabindex="-1" aria-hidden="true">
                    @foreach ($ToGetActiveTechnician as $value)
            ...
            <div class="form-group" data-select2-id="2">
                <label style="font-size: 1.5rem;">Customer Name <span style="color:red;">*</span></label>
                <select class="form-control custom-select2 select2-hidden-accessible" name="customer_id" required style="width: 100%"  data-select2-id="2" tabindex="-1" aria-hidden="true">
                    @foreach ($ToGetCustomerDetails as $value)
                        <option value="{{ $value->customer_id }}" style="font-size: 1.5rem;">{{ $value->fname }} {{ $value->lname }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control" name="job_description" value="{{ old('job_description') }}"  required placeholder="Enter Job Synopsis" style="font-size: 1.5rem; height:700px"></textarea>
            </div>

            <button type="submit" class="btn btn-primary" style="font-size: 1.5rem;">Create New Job</button>
        </form>
    </div>
</div>
