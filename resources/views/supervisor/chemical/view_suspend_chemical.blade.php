...
<div class="min-height-200px">
    <div class="mt-1 mb-5">
        <div class="mb-5">
            <h4>View All Chemical (Suspend)</h4>
        </div>
    </div>
    <div class="card-box mb-30" style="overflow-y: auto;">
        <div class="col-sm-12">
            <table class="data-table table stripe hover nowrap dataTable no-footer dtr-inline ">
                ...
                <tbody>
                @if (is_string($viewSuspendChemical))
                    <tr>
                        <td colspan="5" style="text-align: center; font-size: 1.5rem;">{{ $viewSuspendChemical }}</td>
                    </tr>
                @else
                @php
                    $counter = 0;
                @endphp
                @foreach ($viewSuspendChemical as $value)
                    <tr>
                        ...
                        <td>
                            <a href="{{ url('supervisor/chemical/activate_chemical/' . $value->chemical_id) }}" class="btn btn-sm btn-primary" style="font-size: 1.5rem;">Restore</a>
                        </td>
                    </tr>
                @endforeach
            ...
            <div style="padding: 10px; float: right;">
                {!! $viewSuspendChemical->appends(Illuminate\Support\Facades\Request::except('page'))->links() !!}
            </div>
            @endif
        </div>
    </div>
